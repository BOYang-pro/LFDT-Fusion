...
class DiffForward(nn.Module):
    def __init__(self, dim, ffn_factor, bias):
        super(DiffForward, self).__init__()
...
class DTB(nn.Module):
    ...
    def __init__(self, dim, num_heads, ffn_factor, bias, LayerNorm_type):
        ...
        self.DSA =  DiffAttention(dim, num_heads, bias)
        self.norm2 = LayerNorm(dim, LayerNorm_type)
        self.DFN = DiffForward(dim, ffn_factor, bias)
    ...
    def forward(self, x, t):
        ...
        x = x + self.DSA(self.modulate(self.norm1(x), shift_att, scale_att))
        x = x + self.DFN(self.modulate(self.norm2(x), shift_ffn, scale_ffn))
